    ...
    function consoleLog(word, index){
        ...
        if (youtubeRegex.test(word)){
            ...
            youtubedl.getInfo(word, function(err, info) {
                ...
                let link = 'http://www.meascheese.com/ots/' + info._filename.replace(/ /g, '_') + '.mp3';
                message.channel.send('Audio file available at: ' + link);
            })



            youtubedl.exec(word, ['-x', '--audio-format', 'mp3', '-o', '/var/www/meascheese.com/shawn/public_html/ots/%(title)s.%(ext)s', '--restrict-filenames'], {}, function(err, output){
                if (err) throw err;
